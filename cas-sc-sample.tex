...
In addition to process-based inundation simulation, HydroSim-RF provides an ML-based susceptibility (likelihood) proxy derived from DEM predictors. The implementation computes a compact feature set per cell consisting of: (i) normalized elevation and (ii) normalized slope magnitude. Both predictors are robustly scaled to $[0,1]$ using percentile-based normalization (2nd--98th percentiles) to reduce sensitivity to outliers; slope is estimated from DEM gradients.

Binary training labels are generated from the simulation result by thresholding the simulated water depth (flooded if $h > h_f$). A Random Forest classifier is trained with class balancing to mitigate label imbalance. The trained model outputs a per-cell probability score in $[0,1]$ that is interpreted as a susceptibility proxy conditioned on the simulated labels and predictors (i.e., a screening layer rather than a calibrated probability of observed flooding).
